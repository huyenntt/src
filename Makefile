...
CONFIG_CLEAN_FILES = libpnapi.pc pnapi.cc bison-wrapper.sh
...
am__v_lt_0 = --silent
libpnapi_la_LINK = $(LIBTOOL) $(AM_V_lt) --tag=CXX $(AM_LIBTOOLFLAGS) \
	$(LIBTOOLFLAGS) --mode=link $(CXXLD) $(AM_CXXFLAGS) \
	$(CXXFLAGS) $(libpnapi_la_LDFLAGS) $(LDFLAGS) -o $@
DEFAULT_INCLUDES = -I.
...
SOURCES = $(libpnapi_la_SOURCES)
DIST_SOURCES = $(libpnapi_la_SOURCES)
DATA = $(pkgconfig_DATA)
...
EXTRA_DIST = Makefile.am.customer FlexLexer.h $(BISON_FILES:.yy=.h) \
	$(BISON_FILES:.yy=.cc) $(BISON_FILES:.yy=-location.h) \
	$(BISON_FILES:.yy=-position.h) $(BISON_FILES:.yy=-stack.h)
BUILT_SOURCES = Makefile.am.customer $(BISON_FILES:.yy=-location.h) # \
	bison hack
...
#---------------------------------------------------#
MAINTAINERCLEANFILES = $(BISON_FILES:.yy=.h) $(BISON_FILES:.yy=.cc) \
	$(BISON_FILES:.yy=-location.h) $(BISON_FILES:.yy=-position.h) \
	$(BISON_FILES:.yy=-stack.h)
DISTCLEANFILES = 
...
libpnapi.pc: $(top_builddir)/config.status $(srcdir)/libpnapi.pc.in
	cd $(top_builddir) && $(SHELL) ./config.status $(subdir)/$@
pnapi.cc: $(top_builddir)/config.status $(srcdir)/pnapi.cc.in
	cd $(top_builddir) && $(SHELL) ./config.status $(subdir)/$@
bison-wrapper.sh: $(top_builddir)/config.status $(srcdir)/bison-wrapper.sh.in
	cd $(top_builddir) && $(SHELL) ./config.status $(subdir)/$@
...
libpnapi.la: $(libpnapi_la_OBJECTS) $(libpnapi_la_DEPENDENCIES) 
	$(AM_V_CXXLD)$(libpnapi_la_LINK) -rpath $(libdir) $(libpnapi_la_OBJECTS) $(libpnapi_la_LIBADD) $(LIBS)
...
include ./$(DEPDIR)/formula.Plo
include ./$(DEPDIR)/graph.Plo
include ./$(DEPDIR)/interface.Plo
include ./$(DEPDIR)/io-format.Plo
include ./$(DEPDIR)/marking.Plo
include ./$(DEPDIR)/myio.Plo
